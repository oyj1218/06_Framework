...
function selectCommentList(){
    ...
    // - 자원을 이름으로 구분(REpresentational)하여 자원 상태(State)를 주고 받는 것(Transfer)
    // 물(이름)이란 자원 + 마셔줘(상태) -> 상태에 따라서 행동이 달라짐
    // 물(이름)이란 자원 + 따라줘(상태) -> 상태에 따라서 행동이 달라짐
    // 대신 물이란 자원 대신에 주소를 넣을 것

    // 조회(get), 수정(post), 삽입(put), 삭제(delete) 상태에 따라서 /comment로 한 개로 끝낼 것
    ...
    // 근데 기본적으로 form 태그 get/post만 지원한다
    // 그래서 http 통신에 의미적으로 맞지 않아서 get/post만 사용한다
    
    fetch("/comment?boardNo=" + boardNo) // 요청 주소 이젠, /comment로만 사용할 예정
    // get방식은 주소에 파라미터
...
function deleteComment(commentNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        fetch("/comment",{
            method : "DELETE",
            headers : {"Content-Type" : "application/json"},
            body : commentNo // 값 하나 전달 시에는 JSON 필요 X
            // commentContent만 넘기면 되기에 JSON.stringify(data)으로 안 써도 된다
        })
        .then(resp => resp.text())
        .then(result => {
            if(result > 0){
